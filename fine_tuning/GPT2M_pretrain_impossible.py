...
def prepare_dataset(perturbed_sentences, tokenizer, train_split=0.9, max_length=128):
    """
    Tokenize perturbed sentences for standard causal language modeling.
    No pairs, no prompts, no masking — just perturbed text.
    """
...
def train_model(train_dataset, eval_dataset, config, model_name, output_dir):
    ...
    # From-scratch: load config only, not weights.
    ...
    # For causal LM, the collator shifts input_ids to create labels.
